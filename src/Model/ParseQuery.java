...
public class ParseQuery {
    ...
    public void parse(Query PQ) {
        stopWordsFunc(this.savepath);
        termsQuery = new LinkedHashMap[27];
        for (int i = 0; i < termsQuery.length; i++)
            termsQuery[i] = new LinkedHashMap();
        String first = "";
        String second = "";
        String third = "";
        String fourth = "";
        String month = null;
        String length = null;
        String wordString = PQ.getQueryTitle();
        ...
        for (i = 0; i < words.length + 4; i++) {
            ...
            try {
                ...
                finishDocAlgorithm(termsQuery, savepath);
            ...
            if (finishedDoc) {
                finishedDoc = false;
                // TODO: 21/12/2018 fix here to send to new func in indexer  Itzik
                finishDocAlgorithm(termsQuery, savepath);
            ...
            if (!term.equals("")) {
                ...
                if (term.charAt(0) >= 'A' && term.charAt(0) <= 'Z') {//check the lower / uppercase rule.
                    ...
                    if (indexer.getDictionary()[i].get(lowerTerm) != null) {
                        ...
                        int c = 0;
                        if (termsQuery[i].get(lowerTerm) != null) {
                            c = termsQuery[i].get(lowerTerm);
                            termsQuery[i].put(lowerTerm, ++c);
                        } else {
                            if (indexer.getDictionary()[i].get(lowerTerm) != null) {
                                termsQuery[i].put(lowerTerm, 1);
                            } else {
                                if (termsQuery[i].get(term.toUpperCase()) != null) {
                                    c = termsQuery[i].get(term.toUpperCase());
                                    termsQuery[i].put(term.toUpperCase(), ++c);
                                } else {
                                    termsQuery[i].put(term.toUpperCase(), 1);
                                }
                            }
                        }
                    } else if (term.charAt(0) >= 'a' && term.charAt(0) <= 'z') {
                        if (termsQuery[i].get(term) == null) {
                            termsQuery[i].put(term, 1);
                        } else {
                            Integer c = termsQuery[i].get(term);
                            termsQuery[i].put(term, ++c);
                        }
                    } else {
                        //number, special chars..
                        int x = correctDictionaryCell(term);
                        if (termsQuery[x].get(term) != null) {
                            int c = termsQuery[x].get(term);
                            termsQuery[x].put(term, ++c);
                        } else {
                            termsQuery[x].put(term, 1);
                        }
                    }
                }
            }
